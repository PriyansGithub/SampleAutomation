...
public class VerifyFileUploadWithCorrectData extends BaseTest {
    @Test
    public void verifyFileUploadWithCorrectData() {
        ClerkDashboardPage clerkDashboardPage=new ClerkDashboardPage(driver);
        try {
            HashMap<String, String> testData = actions.getTestData("VerifyFileUploadWithCorrectData");
            loginPage.login(testData.get("username"), testData.get("password"));
            homePage.clickUploadCSV();
            clerkDashboardPage.verifyNoUploadMessagePresent();
            String uploadMessage = clerkDashboardPage.uploadCSVAndReturnMessage(testData.get("filePathWithCorrectData"));
